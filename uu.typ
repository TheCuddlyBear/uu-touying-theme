...
#let slide(
  ...
  let header(self) = {
    ...
    grid(
      ...
      block(
        inset: (x: 1.1em, y: .5em),
        text(fill: black, weight: "bold", size: 1.2em, utils.call-or-display(self, self.store.header))
      ),
    )
  }
  let footer(self) = {
...
#let title-slide(
  ...
  let body = {
    place(left, image("logo.png"), dx: -2.5em, dy: -2.5em)
    if info.institution != none {
      place(center, text(size: .6em, info.institution))
    }
    place(right, text(size: .6em)[#datetime.today().display()])
    std.align(
      center + horizon,
       stack(
      dir: ttb,
      ...
      block(
        ...
        stack(
          dir: ttb,
          text(size: 2em, style: "italic", weight: "bold", info.title),
          block(height: 30pt),
          if info.subtitle != none {
            text(size: 0.8em, weight: "thin", info.subtitle)
          },
        )
      ),
      if extra != none {
        place(center, text(size: 1.5em, weight: "bold", extra))
      },
      if info.authors != none {
        place(center, text(size: 0.8em, weight: "bold", info.authors.join(", ")))
      },
      if info.short-subtitle != none and info.short-subtitle != auto {
        place(center, text(size: 0.6em, weight: "thin", info.short-subtitle))
      },
    )
    )
  }
...
#let new-section-slide(config: (:), level: 1, numbered: true, body) = touying-slide-wrapper(self => {
  let slide-body = {
    place(left, image("logoKleur.png"), dx: -2.5em, dy: -2.5em)
    if self.info.institution != none {
      place(center, text(size: .6em, self.info.institution))
    }
    place(right, text(size: .6em)[#datetime.today().display()])
